...
//		})
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if #available(iOS 14, *) {
			...
			cell.detailTextLabel?.text = valueDescription(forElement: elem)
		}
	...
	override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
		...
		return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
			let copyKey = UIAction(title: .localized("Copy Key")) { _ in
				UIPasteboard.general.string = text
			}
			
			let copyValue = UIAction(title: .localized("Copy Value")) { _ in
				UIPasteboard.general.string = elem.description
			}
			
			return UIMenu(children: [copyKey, copyValue])
		}
	}
